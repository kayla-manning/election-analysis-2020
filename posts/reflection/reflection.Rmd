...
- Despite overpredicting Joe Biden's vote share in most states, the model *underestimated* Joe Biden's performance in the only three states with the incorrect predicted winner. Essentially, the model overestimated Joe Biden's vote share in general but underestimated it in the states with incorrect point predictions.

The maps and table, included below, illustrate the aforementioned patterns.
...
ggplotly(state_pred_compare %>% 
  ...
  labs(x = "",
       y = "",
       fill = "Difference between Biden's Actual \nand Predicted Two-Party Vote Share",
       title = "Prediction Error in my Forecast") +
  theme_hodp() +
  theme(axis.line = element_blank(),
        axis.text = element_blank(),
        axis.text.x = element_blank()),
...
ggplotly(errors %>% 
  ...
  facet_wrap(~ source) +
  labs(x = "Error = Actual Democratic Two-Party Vote Share - Predicted",
       y = "Count",
       title = "Distribution of State-Level Errors \nin Forecast Models") +
  theme_hodp()) 
...
To assess this hypothesis, I could reconstruct the model, following the same procedures as outlined in my [final prediction](https://kayla-manning.github.io/gov1347/posts/final.html). I would use the same data from 1992-2016 and include this variable that captures the state-level changes in voting patterns between elections. Once I have constructed this new model, I could assess its validity in several ways:
