...
tibble(Candidate = c("Biden", "Trump"),
       `Hypothetical Electoral Votes` = c(ev$biden_ev, ev$trump_ev),
       `Actual Electoral Votes` = c(306, 232),
       `Hypothetical Two-Party Popular Vote` = c(paste(round(pv2p$d_pv2p * 100, 1),
                                                       "%"), paste(round(pv2p$r_pv2p * 100, 1), "%")),
       `Actual Two-Party Popular Vote` = c("51.3%", "46.9%")) %>% 
  kable(align = rep("c", 5)) %>% 
...
[^poll-weeks]: While this includes some data from after COVID-19 came to the United States, I had to expand the window of time to get a large enough sample size of polls for each state. The model would not run if I restricted the polling window any more.
...
[^death-day]: I used total COVID case and death tallies as of Election Day.
