...
deaths <- covid_nat %>% 
  ...
  geom_jitter() +
  theme_minimal() +
  labs(x = "Daily Increase in US COVID-19 Deaths",
       y = "Average Polling Support",
       title = "Polling Response to COVID-19 Deaths",
       color = "Candidate") +
...
ggsave("../figures/shocks/covid_polls.jpg")
...
covid_dat <- read_csv("../data/covid-19-polls-master-538/covid_approval_polls_adjusted.csv") %>% 
  ...
  inner_join(poll_2020_df %>% 
               filter(party == "democrat"), by = c("enddate" = "poll_date")) %>% 
...
covid_dat %>% 
 ...
 covid_dat %>% 
  ggplot(aes(approve_adjusted, avg_support)) +
  geom_point() +
  ...
  labs(x = "Approval of Trump's Response to COVID-19",
       ...
       subtitle = "Correlation of -0.473",
       caption = "Both datasets came from FiveThirtyEight")
...
prob_table %>% 
  ...
  mutate(d_pv2p = sim_dvotes_2020 / (sim_rvotes_2020 + sim_dvotes_2020),
         r_pv2p = 1 - d_pv2p) %>% 
  summarise(d_pv2p = mean(d_pv2p, na.rm = TRUE) * 100,
            r_pv2p = mean(r_pv2p, na.rm = TRUE) * 100) %>% 
...
ggsave("../figures/shocks/margin_map.jpg")
