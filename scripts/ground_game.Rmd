  ...
  inner_join(state_votes %>% 
               ...
               mutate(prev_winner = lag(winner) %>% as.character(),
                      ...
                      prev_rep = case_when(prev_winner == "rep" ~ "yes",
                                           prev_winner == "dem" ~ "no",
                                           prev_winner == "none" ~ "none"),
                      consec_dem = ave(prev_dem, rleid(prev_dem, state), FUN = seq_along),
                      consec_rep = ave(prev_rep, rleid(prev_rep, state), FUN = seq_along))) %>% 
...
prob_table %>% 
  ...
  group_by(state) %>% 
  summarise(prob_biden = mean(biden_wins, na.rm = TRUE)) %>% 
...
nat_prob_table <- tibble(vep = rep(vep_national, 100000), 
                         ...
                         sim_dem = 0, 
                         sim_rep = 0) %>% 
  mutate(vep = map_dbl(vep, ~ rnorm(1, mean = ., sd = vep_sd)))
...
nat_prob_table %>% 
  ...
  pivot_longer(cols = everything(), names_to = "party", values_to = "simulation") %>% 
  ggplot(aes(simulation, fill = party)) +
  geom_histogram(bins = 1000) +
  theme_minimal() +
  scale_fill_manual(labels = c("Biden", "Trump"),
                    values = c(muted("blue"), "red3")) +
  labs(x = "Simulated Vote Counts",
       y = "Number of Occurrences",
       fill = "Candidate",
       title = "Simulated Nationwide Election Results",
       caption = "Predicted using polling 3 weeks out, Q2 GDP, incumbency, 
