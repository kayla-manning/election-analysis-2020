...
person_spending %>% 
  ...
  coord_flip() +
  theme_minimal() +
  scale_fill_manual(values = c("gray", muted("red2"), "black")) +
...
ggarrange(fb_map + theme(legend.position = "none"), 
          tv_map + theme(legend.position = "none"), ncol = 2) %>% 
  annotate_figure(top = text_grob("Relative Advertising in Battleground States", size = 20))

ggsave("../figures/ads/fb_tv_maps.jpg")
...
plot_usmap(data = state_predictions %>% drop_na(winner), regions = "states", values = "winner") +
  scale_fill_manual(values = c(muted("blue"), muted("red3")), breaks = c("Biden", "Trump")) +
  theme_void() +
  labs(fill = "Winner") +
  labs(title = "Predicted 2020 Election Results",
       caption = "Using a multiple linear regression mapping two-party vote share 
