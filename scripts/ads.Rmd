...
person_spending %>% 
  ...
  coord_flip() +
  theme_minimal() +
  scale_fill_manual(values = c(muted("red3"), muted("red2"), "gray")) +
...
# Biden ad spending in swing states 
...
plot_usmap(regions = "states", data = biden_swing,
  ...
  theme_void() +
  labs(title = "Facebook Spending Margin from 9/6/2020-10/5/2020",
...
plot_usmap(data = swing_tv, regions = "states", values = "biden_margin", include = states) +
  scale_fill_gradient2(
    high = muted("blue"),
    mid = "white",
    low = muted("red3"),
  ...
  theme_void() +
  labs(title = "TV ad margin from 9/5/2020-9/27/2020",
       fill = "Biden's Margin")
...
ads_2000s %>% 
  mutate(year = year(air_date),
         month = month(air_date)) %>% 
  # filter(year == 2000) %>% 
  group_by(air_date, year, party, month) %>% 
  summarise(total_cost = mean(total_cost)) %>% 
  ggplot(aes(month, total_cost, color = party)) +
  geom_point() +
  geom_line() +
  facet_wrap(~ year)
...
# planning on comparing the numbers for each candidate with the ad spend in states

region_search %>% 
  pivot_longer(2:ncol(region_search), names_to = "candidate", values_to = "searches") %>% 
  rename(state = region) %>% 
  plot_usmap(data = ., regions = "states", values = "searches") +
  scale_fill_gradient(
    high = muted("red3"),
    low = "white"
  ) +
...
# plot of google searches over time

trends_time %>% 
  pivot_longer(2:ncol(trends_time), names_to = "candidate", values_to = "searches") %>% 
  ggplot(aes(week, searches, color = candidate)) +
  geom_line() +
  theme_classic()
...
files <- c("updated", "top-keywords-history", "geo-spend", "creative-stats", "campaign-targeting",
...
# looking at overall advertiser spending

geo_spend <- `geo-spend`

geo_spend %>% 
  filter(country == "US") %>% 
  mutate(country_subdivision_primary = str_remove(country_subdivision_primary, "US-")) %>% 
  rename(state = country_subdivision_primary) %>% 
  select(state, spend_usd) %>% 
  plot_usmap(data = ., regions = "states", values = "spend_usd", include = states) +
  scale_fill_gradient(high = muted("red3"), low = "white")
...
candidate_google_spend %>% 
  ...
  labs(fill = "Biden's Google Spending Margin",
       title = "Google Advertising Spending Margin in Swing States",
       caption = "As of 10/8/2020") +
  theme_void()
