...
prob_table %>% 
  ...
  summarise(biden_win = sum(biden_victory),
            trump_win = sum(trump_victory),
            tossup = sum(tossup)) 
...
prob_table %>% 
  ...
  labs(title = "Simulated National Two-Party Popular Vote",
       ...
       y = "Count",
       subtitle = "Donald Trump Wins the Popular Vote in 0.421% of Simulations",
       fill = "Candidate") +
  theme_minimal()
...
prob_table %>% 
  ...
  mutate(close = abs(biden_wins - 0.5)) %>% 
  arrange(close) %>% 
