...
prob_table <- tibble(state = c(rep(dem_states, each = 100000), rep(bg_states, each = 100000), rep(rep_states, each = 100000)), 
...
for (state_type in state_types) {
  ...
  for (s in state_type) {
    ...
    for (p in parties) {
      ...
      # 
      # # just getting polls from pollsters graded from A+ to B/C (total
      # # possibilities are "B"   "C"   "B/C" "C-"  "A+"  "A"   "B-"  "D-"  NA
      # # "A/B" "C+"  "A-"  "B+"  "C/D")
      # 
      # # poll <- state_polls_2020 %>% 
      # #   filter(state == s,
      # #          weeks_left %in% c(1, 2, 3, 4),
      # #          str_detect(fte_grade, "A") | str_detect(fte_grade, "B")) %>% 
      # #   group_by(party) %>% 
      ...
      # # if (length(poll$party) == 0) {
        ...
        filter(state == s,
               weeks_left %in% c(1, 2, 3, 4),
               str_detect(fte_grade, "A") | str_detect(fte_grade, "B") | str_detect(fte_grade, "C"),
               !str_detect(fte_grade, "D")) %>% 
        group_by(party) %>% 
        summarise(avg_support = mean(pct, na.rm = TRUE)) 
      # }
...
prob_table %>% 
  ...
  tab_header(title = "Uncertainty Within States",
             subtitle = "Ordered in terms of closeness, as of 10/30/2020") %>% 
  cols_label(state = "State", pct_biden_win = "Probability of Biden Victory") %>% 
  tab_footnote(footnote = "For states not listed, the forecasted winner won every simulated election.",
...
ev_close <- ev_uncertainty %>% 
  ...
  ggplot(aes(ev, fill = candidate)) +
  geom_histogram(bins = 100, alpha = 0.75, position = "identity") +
  labs(title = "Simulated Electoral Vote Counts",
       x = "Electoral Votes",
       y = "Count",
       fill = "Candidate",
       subtitle = "Donald Trump Wins the Electoral College in 38.875% of Simulations") +
  theme_minimal() +
...
pop_close <- prob_table %>% 
  ...
  labs(title = "Simulated National Two-Party Popular Vote",
       ...
       y = "Count",
       subtitle = "Donald Trump Wins the Popular Vote in 0.668% of Simulations",
       fill = "Candidate") +
  theme_minimal()
...
ggarrange(pop_close, ev_close, ncol = 1, common.legend = TRUE, legend = "right")
...
# state-by-state distributions by line plot instead
...
prob_table %>% 
  ...
  mutate(d_pv2p = sim_dvotes_2020 / (sim_dvotes_2020 + sim_rvotes_2020),
         r_pv2p = 1 - d_pv2p) %>% 
  pivot_longer(cols = c(d_pv2p, r_pv2p), names_to = "party") %>% 
  ggplot(aes(state, value, fill = party)) +
  geom_point(bins = 1000, alpha = 0.5, position = "identity") +
  scale_fill_manual(breaks = c("d_pv2p", "r_pv2p"),
                    labels = c("Biden", "Trump"),
                    values = c(muted("blue"), "red3")) +
  labs(title = "Simulated Two-Party Popular Vote in Battleground States",
...
ggarrange(partisan  + theme(legend.position = "none"), 
  ...
  annotate_figure(text_grob("95% Confidence Intervals for Two-Party Vote Share",
                            size = 15))
