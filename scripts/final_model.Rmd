...
dem_states <- c("CO", "VA", "CA", "CT", "DE", "HI", "IL", "MD", "MA", "NJ", "NY", "OR", "RI", "VT", "WA")
...
outsamp_results <- tibble(year = rep(years, each = 100),
       ...
       state = c(rep(c(bg_states, rep_states, dem_states), times = 14)),
       prediction = 0) %>% 
  inner_join(vep %>% 
               filter(year == 2016) %>% 
               mutate(state = state.abb[match(state, state.name)]) %>% select(state, VEP))
i = 1
    ...
    for (party in parties) {
      for (state_type in state_types) {
        for (state in state_type) {
            ...
            states <- state_type[!state_type %in% state]
            
            prev_margin <- state_votes %>% 
                              filter(state == state, 
                                     year == ifelse(year == 1992, 1992, year - 4)) %>% 
                              slice(1) %>% 
                              pull(dem_margin)
            ...
            outsamp_results[which(outsamp_results$year == year & 
                                    outsamp_results$party == party & 
                                    outsamp_results$state == state), 4] <- predict(
                  outsamp_mod, tibble(avg_state_poll = ifelse(party == "democrat", 
...
prob_table %>% 
  mutate(group = rep(1:1000, times = 50)) %>% 
  group_by(group) %>% 
