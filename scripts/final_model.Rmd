...
prob_table <- tibble(state = c(rep(dem_states, each = 10000), rep(bg_states, each = 10000), rep(rep_states, each = 10000)), 
  ...
  inner_join(vep_std) %>% 
  mutate(VEP = map2_dbl(VEP, vep_sd, ~ rnorm(1, .x, 2 * .y)),
         d_fit_link = 0,
         r_fit_link = 0,
         d_se_link = 0,
         r_se_link = 0)
...
for (state_type in state_types) {
  ...
  for (s in state_type) {
    ...
    for (p in parties) {
      ...
      prob_table[which(prob_table$state == s), z] <- predict(
        mod, tibble(avg_state_poll = poll,
                        incumbent = incumbency,
                        gdp_growth_qt = q1_gdp,
                        black_change = state_votes %>% 
      ...
      prob_table[which(prob_table$state == s), x] <- predict(
        mod, tibble(avg_state_poll = poll,
                        incumbent = incumbency,
                        gdp_growth_qt = q1_gdp,
                        black_change = state_votes %>% 
...
prob_table <- prob_table %>% 
  ...
  mutate(d_fit_resp = ilink(prob_dvote),
         conf.d.high = ilink(d_fit_link + (qnorm(0.005, lower.tail = FALSE) * d_se_link)) * VEP,
         conf.d.low = ilink(d_fit_link - qnorm(0.005, lower.tail = FALSE) * d_se_link) * VEP,
         r_fit_resp = ilink(prob_rvote),
         conf.r.high = ilink(r_fit_link + (qnorm(0.005, lower.tail = FALSE) * r_se_link)) * VEP,
         conf.r.low = ilink(r_fit_link - (qnorm(0.005, lower.tail = FALSE) * r_se_link)) * VEP)
...
# PA, WI, AZ. of these, FiveThirtyEight incorrectly predicted FL, NC, MI, PA, and
...
prob_table %>% 
  select(state, sim_dvotes_2020, sim_rvotes_2020, conf.d.low, 
         conf.d.high, conf.r.low, conf.r.high) %>% 
  mutate(total = sim_dvotes_2020 + sim_rvotes_2020) %>% 
  pivot_longer(cols = 2:7, names_to = "measure", values_to = "count") %>% 
  mutate(measure = recode(measure, sim_dvotes_2020 = "d",
                          sim_rvotes_2020 = "r")) %>% 
  group_by(state, measure) %>% 
  summarise(mean = mean(count, na.rm = TRUE)) %>% 
  pivot_wider(names_from = measure, values_from = mean) %>%
  mutate(margin = (d - r) / (d + r),
         margin.low = (conf.d.low - conf.r.low) / (conf.d.low + conf.r.low),
         margin.high = (conf.d.high - conf.r.high) / (conf.d.high + conf.r.high)) %>% 
  filter(abs(margin) < 0.05) %>% 
  ggplot(aes(fct_reorder(factor(state), margin), margin)) +
  geom_point() +
  geom_errorbar(aes(ymin = margin.low, ymax = margin.high)) + 
  coord_flip() +
  theme_minimal()
...
r_mod <- state_votes %>%
        ...
        glm(cbind(r, VEP - r) ~ avg_support + incumbent + gdp_growth_qt + 
              prev_dem_margin + consec_rep + black_change,
            data = ., family = "binomial") 
...
state_votes %>%
        filter(party == "democrat",
               ...
               quarter == 1) %>% 
        glm(cbind(r, VEP - r) ~ avg_support + incumbent + gdp_growth_qt + 
              prev_dem_margin + consec_dem + black_change,
            data = ., family = "binomial") %>% 
  summary()
...
# confidence intervals for vote margins
...
prob_table %>% 
  ...
  mutate(d_pv2p = sim_dvotes_2020 / (sim_rvotes_2020 + sim_dvotes_2020),
         ...
         d_margin = d_pv2p - r_pv2p) %>% 
  group_by(state) %>% 
  summarise(margin_se = sd(d_pv2p) / sqrt(n()),
            d_pv2p = mean(d_pv2p),
            r_pv2p = mean(r_pv2p),
            margin = mean(d_margin)) %>% 
  mutate(conf.low = d_pv2p - 1.96 * margin_se,
         conf.high = d_pv2p + 1.96 * margin_se) %>% 
  filter(d_pv2p > 0.49 & d_pv2p < 0.51) %>% 
  ggplot(aes(fct_reorder(factor(state), d_pv2p), d_pv2p)) +
  geom_point() +
