  ...
  trump_poll <- polls %>% 
    ...
    pull(avg_support)  
      ...
      dem_mod <- state_votes %>%
        ...
        glm(cbind(d, VEP - d) ~ avg_polls * incumbent + gdp_growth_qt +
              prev_dem_margin + black_change,
            data = ., family = "binomial")
  
      prob_table[which(prob_table$state == s), 2] <- predict(
        dem_mod, tibble(avg_polls = biden_poll,
                        incumbent = FALSE,
      ...
      rep_mod <- state_votes %>%
        ...
        glm(cbind(r, VEP - r) ~ avg_polls * incumbent + gdp_growth_qt + 
              prev_dem_margin + black_change,
            data = ., family = "binomial")
      
      prob_table[which(prob_table$state == s), 3] <- predict(
        rep_mod, tibble(avg_polls = trump_poll,
                        incumbent = TRUE,
                        gdp_growth_qt = q1_gdp,
                        black_change = state_votes %>% 
                          filter(state == s, year == 2016) %>% 
...
prob_table %>% 
  mutate(diff_sims = sim_dvotes_2020 - sim_rvotes_2020) %>% 
  group_by(state) %>% 
  mutate(mean_diff = mean(diff_sims),
            sd_diff = sd(diff_sims),
            conf.low = diff_sims - 1.96 * sd_diff,
            conf.high = diff_sims + 1.96 * sd_diff) %>% 
  ungroup() %>% 
  mutate(insignificant = case_when(conf.low < 0 & conf.high > 0 ~ TRUE,
                                   TRUE ~ FALSE)) %>% 
  select(state, sim_dvotes_2020, sim_rvotes_2020, diff_sims, conf.low, conf.high, insignificant) %>% 
  filter(insignificant == TRUE) %>% 
  count(state)
