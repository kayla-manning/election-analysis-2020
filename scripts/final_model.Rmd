...
for (state_type in state_types) {
  ...
  for (s in state_type) {
    ...
    for (p in parties) {
      ...
      # weighing higher-rated polls by more; A-rated by 20x, B-rated by 10x,
      # C-rated by 6x
      
      poll <- state_polls_2020 %>% 
        filter(state == s,
               weeks_left %in% c(1, 2, 3, 4)) %>% 
        mutate(ntimes = case_when(str_detect(fte_grade, "C") ~ 6,
          str_detect(fte_grade, "B") ~ 10,
          str_detect(fte_grade, "A") ~ 20,
                                  TRUE ~ 1)) 
...
ev_close <- ev_uncertainty %>% 
  ...
  mutate(result = case_when(ev <= 270 ~ "Biden",
                            ev > 270 ~ "Trump",
                            ev == 269 ~ "Tie")) %>% 
  ...
  labs(title = "Simulated Electoral Vote Counts",
       ...
       y = "Count",
       fill = "Winner",
       subtitle = "Donald Trump Wins the Electoral College in 33.71% of Simulations") +
...
pop_close <- prob_table %>% 
  ...
  labs(title = "Simulated National Two-Party Popular Vote",
       ...
       y = "Count",
       subtitle = "Donald Trump Wins the Popular Vote in 0.050% of Simulations",
       fill = "Candidate") +
  theme_minimal()
